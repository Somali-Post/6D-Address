...
const RegisterMapPage: React.FC = () => {
    ...
    const [isSendingOtp, setIsSendingOtp] = useState<boolean>(false); // Separate loading state for sending OTP
    ...
    const handleSendOtp = async () => { // Make async
        ...
        if (!mobileNumber || (mobileInputRef.current && !mobileInputRef.current.checkValidity())) {
            setError("Please enter a valid Somali mobile number first.");
            mobileInputRef.current?.focus();
            return;
        }
        setIsSendingOtp(true); // Use specific loading state
        ...
        try {
            ...
            if (!response.ok) {
                // Handle errors from the backend (e.g., 400 Bad Request)
                throw new Error(data.message || `Failed to send OTP: ${response.statusText}`);
            }

            // Success (even simulated)
            console.log('Backend response for send-otp:', data.message);
            setCurrentPhase('verifyOtp'); // Move to OTP verification phase
    ...
    const handleVerifyOtp = async () => { // Make async
        ...
        try {
            ...
            if (!response.ok) {
                // Handle backend verification failure (e.g., 400 Incorrect code)
                throw new Error(data.message || `Failed to verify OTP: ${response.statusText}`);
            }

            // Success
            console.log("Backend OTP verification successful:", data.message);
            setIsMobileVerified(true);      // Set verified state
            setCurrentPhase('enterDetails'); // Go back to details phase, submit button will be enabled
            setError(null);                 // Clear any previous OTP errors

        } catch (err: any) {
            console.error("Error verifying OTP:", err);
            setError(err.message || "Could not verify code. Please try again.");
            setIsMobileVerified(false); // Ensure verification state is false on error
        } finally {
    ...
    if (isLoaded && google?.maps?.SymbolPath && google?.maps?.Point) { customMarkerOptions = { path: 'M 0,0 C -2,-20 -10,-22 -10,-30 A 10,10 0 1,1 10,-30 C 10,-22 2,-20 0,0 z', fillColor: '#2563EB', fillOpacity: 1.0, strokeColor: '#ffffff', strokeWeight: 1.5, scale: 1.0, anchor: new google.maps.Point(0, -35) }; }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-x-8 gap-y-6">
        ...
        <div className="lg:col-span-2 relative">
         ...
         <div className="lg:col-span-1 flex flex-col space-y-5">
             {currentPhase === 'selectLocation' && ( <> <h2 className="text-2xl font-semibold text-gray-800">Details</h2> <div className="bg-blue-50 border border-blue-200 text-blue-700 px-4 py-3 rounded-lg text-sm flex items-center space-x-3"> <FiInfo className="h-5 w-5 flex-shrink-0"/> <span>Click map to start.</span> </div> </> )}
            {(currentPhase === 'enterDetails' || currentPhase === 'verifyOtp' || currentPhase === 'submitted') && ( <> <h2 className="text-2xl font-semibold text-gray-800">{currentPhase === 'submitted' ? 'Registration Summary' : '2. Enter Your Details'}</h2> {isLoadingGeocode && <LoadingSpinner message="Getting details..." />} {!isLoadingGeocode && generatedCode && ( <div className="bg-slate-50 p-4 rounded-lg border border-slate-200"> <label className="block text-xs font-medium text-gray-500 mb-1 uppercase tracking-wider">Your 6D Code</label> <div className="flex items-center justify-between"> <strong className="text-2xl font-mono text-blue-700 tracking-wider">{generatedCode}</strong> <button onClick={handleCopyCode} title="Copy" className="p-2 text-gray-400 hover:text-blue-600 rounded-full focus:outline-none focus:ring-2 focus:ring-blue-300"> {copied ? <FiCheck className="text-green-600 h-5 w-5" /> : <FiCopy className="h-5 w-5"/>} </button> </div> </div> )} {!isLoadingGeocode && addressContext && ( <div className="bg-gray-50 p-3 rounded-lg border border-gray-200 text-sm"> <p><span className="font-medium text-gray-600">Approx. Location:</span> {addressContext.sublocality || 'N/A'}, {addressContext.locality || 'N/A'}</p> {addressContext?.error && <p className='text-red-600 text-xs mt-1'>({addressContext.error})</p>} </div> )}
            {/* Form Section (Only if not submitted) */}
            {currentPhase !== 'submitted' && (
            <form onSubmit={handleSubmitRegistration} className="space-y-4">
                {/* Name Input */}
                <Input ref={nameInputRef} label="Full Name" id="name" type="text" placeholder="Your Full Name" value={name} onChange={handleNameChange} required autoComplete="name" disabled={isSubmitting || currentPhase === 'verifyOtp'} icon={<FiUser className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400"/>} className="pl-10" />
                {/* Mobile Input Section */}
                <div className="space-y-1">
                    <Input ref={mobileInputRef} label="Mobile Number" id="mobileNumber" type="tel" placeholder="61xxxxxxx" value={mobileNumber} onChange={handleMobileChange} required pattern="^(?:\+?252)?(61|62|63|65|68|90)\d{7}$" title="Valid Somali mobile (e.g. 61xxxxxxx)" disabled={isSubmitting || currentPhase === 'verifyOtp' || isMobileVerified } aria-describedby="mobile-helper-text" icon={<FiPhone className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400"/>} className="pl-10"/>
                    <div className='flex justify-between items-center mt-1'>
                        <p id="mobile-helper-text" className="text-xs text-gray-500">Verification required.</p>
                        {!isMobileVerified && currentPhase !== 'verifyOtp' && ( <Button type="button" variant="secondary" onClick={handleSendOtp} disabled={!mobileNumber || (mobileInputRef.current && !mobileInputRef.current.checkValidity()) || isSubmitting || isSendingOtp} isLoading={isSendingOtp} className="text-xs px-3 py-1" > Send Code </Button> )}
                        {isMobileVerified && ( <span className="text-xs text-green-600 font-medium flex items-center"><FiCheckCircle className="mr-1"/>Verified</span> )}
                    </div>
                </div>
                {/* OTP Section */}
                {currentPhase === 'verifyOtp' && ( <div className="p-4 border border-blue-200 rounded-lg bg-blue-50 space-y-3"> <p className='text-sm font-medium text-blue-800'>Enter code sent to {mobileNumber}</p> <Input ref={otpInputRef} label="Verification Code" id="otp" type="text" inputMode='numeric' maxLength={4} placeholder="1234" value={otp} onChange={handleOtpChange} required disabled={isVerifyingOtp || isMobileVerified} className="tracking-[0.5em] text-center font-medium text-lg" icon={<FiLock className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400"/>} /> <Button type="button" variant="primary" onClick={handleVerifyOtp} isLoading={isVerifyingOtp} disabled={isVerifyingOtp || !otp || otp.length < 4} className="w-full" > Verify Code </Button> <Button type="button" variant="secondary" onClick={() => { setCurrentPhase('enterDetails'); setError(null); }} disabled={isVerifyingOtp} className="w-full text-xs" > Cancel </Button> </div> )}
                {/* Final Submit Button */}
                {currentPhase === 'enterDetails' && isMobileVerified && ( <Button type="submit" variant="primary" disabled={isSubmitButtonDisabled} isLoading={isSubmitting} className="w-full" > Submit Registration </Button> )}
             </form>
             )}
             </> )}
            {/* Success Message */}
            {currentPhase === 'submitted' && ( <div className="bg-green-50 border border-green-300 text-green-800 px-5 py-4 rounded-lg text-center space-y-2" role="alert"> <FiCheckCircle className="h-8 w-8 mx-auto text-green-600"/> <h3 className="font-semibold text-lg">Registration Complete!</h3> <p className="text-sm">Code <span className="font-mono font-bold">{generatedCode}</span> linked to <span className="font-medium">{mobileNumber}</span>.</p> <Button variant='secondary' className='mt-4 text-xs' onClick={handleRegisterAnother}>Register another location?</Button> </div> )}
