...
const RegisterMapPage: React.FC = () => {
    ...
    const [isSendingOtp, setIsSendingOtp] = useState<boolean>(false); // THIS is used below
    const [isVerifyingOtp, setIsVerifyingOtp] = useState<boolean>(false); // THIS is used below
    ...
    // --- Backend Interaction Callbacks ---

    // ***********************************************
    // ****** FULL IMPLEMENTATION RESTORED HERE ******
    // ***********************************************
    const handleSendOtp = async () => {
        ...
        setIsSendingOtp(true); // <-- USED HERE
        ...
        try {
            ...
            setIsSendingOtp(false); // <-- USED HERE
        }
    };
    ...
    const handleVerifyOtp = async () => {
        ...
        setIsVerifyingOtp(true); // <-- USED HERE
        ...
        try {
            ...
            setIsVerifyingOtp(false); // <-- USED HERE
        }
    };
    ...
    const handleSubmitRegistration = async (event: React.FormEvent) => {
        ...
        if (!generatedCode || !selectedPosition || !name || !mobileNumber || !isMobileVerified) {
            ...
            else if (!isMobileVerified) { /* Maybe focus send OTP btn? */ }
            return;
        }
        ...
        const registrationData = {
            name, mobile: mobileNumber, code6D: generatedCode,
    ...
    const MAP_OPTIONS = useMemo((): google.maps.MapOptions => ({
        ...
        minZoom: 5, gestureHandling: 'greedy', clickableIcons: false,
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-x-8 gap-y-6">
        ...
        {/* Details Column */}
         <div className="lg:col-span-1 flex flex-col space-y-5">
            ...
            {currentPhase !== 'submitted' && ( <form onSubmit={handleSubmitRegistration} className="space-y-4"> <Input ref={nameInputRef} label="Full Name" id="name" type="text" placeholder="Your Full Name" value={name} onChange={handleNameChange} required autoComplete="name" disabled={isSubmitting || currentPhase === 'verifyOtp'} icon={<FiUser className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400"/>} className="pl-10" /> <div className="space-y-1"> <Input ref={mobileInputRef} label="Mobile Number" id="mobileNumber" type="tel" placeholder="61xxxxxxx" value={mobileNumber} onChange={handleMobileChange} required pattern="^(?:\+?252)?(61|62|63|65|68|90)\d{7}$" title="Valid Somali mobile (e.g. 61xxxxxxx)" disabled={isSubmitting || currentPhase === 'verifyOtp' || isMobileVerified } aria-describedby="mobile-helper-text" icon={<FiPhone className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400"/>} className="pl-10"/> <div className='flex justify-between items-center mt-1'> <p id="mobile-helper-text" className="text-xs text-gray-500">Verification required.</p> {!isMobileVerified && currentPhase !== 'verifyOtp' && ( <Button type="button" variant="secondary" onClick={handleSendOtp} disabled={!mobileNumber || (mobileInputRef.current && !mobileInputRef.current.checkValidity()) || isSubmitting || isSendingOtp} isLoading={isSendingOtp} className="text-xs px-3 py-1" > Send Code </Button> )} {isMobileVerified && ( <span className="text-xs text-green-600 font-medium flex items-center"><FiCheckCircle className="mr-1"/>Verified</span> )} </div> </div> {currentPhase === 'verifyOtp' && ( <div className="p-4 border border-blue-200 rounded-lg bg-blue-50 space-y-3"> <p className='text-sm font-medium text-blue-800'>Enter code sent to {mobileNumber}</p> <Input ref={otpInputRef} label="Verification Code" id="otp" type="text" inputMode='numeric' maxLength={4} placeholder="1234" value={otp} onChange={handleOtpChange} required disabled={isVerifyingOtp || isMobileVerified} className="tracking-[0.5em] text-center font-medium text-lg" icon={<FiLock className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400"/>} /> <Button type="button" variant="primary" onClick={handleVerifyOtp} isLoading={isVerifyingOtp} disabled={isVerifyingOtp || !otp || otp.length < 4} className="w-full" > Verify Code </Button> <Button type="button" variant="secondary" onClick={() => { setCurrentPhase('enterDetails'); setError(null); }} disabled={isVerifyingOtp} className="w-full text-xs" > Cancel </Button> </div> )} {currentPhase === 'enterDetails' && isMobileVerified && ( <Button type="submit" variant="primary" disabled={isSubmitButtonDisabled} isLoading={isSubmitting} className="w-full" > Submit Registration </Button> )} </form> )} </> )}
